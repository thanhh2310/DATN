...
public interface UserInteractionRepository extends JpaRepository<UserInteraction, Long> {
    ...
    @Query("SELECT ui FROM UserInteraction ui WHERE ui.userId = :userId AND ui.product.id = :productId ORDER BY ui.createdAt DESC LIMIT 1")
    UserInteraction findLatestByUserIdAndProductId(@Param("userId") Integer userId, @Param("productId") Integer productId);
    ...
    @Query("SELECT ui.product.id, SUM(ui.interactionWeight) as score " +
           "FROM UserInteraction ui " +
           "WHERE ui.userId = :userId AND ui.product IS NOT NULL " +
           "GROUP BY ui.product.id ORDER BY score DESC")
